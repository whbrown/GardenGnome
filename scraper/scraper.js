...
(async () => {
  ...
  await page.goto(
    'https://www.rhs.org.uk/Plants/18919/i-Viburnum-opulus-i/Details'
  );
  let plantData = await page.evaluate(() => {
    ...
    const grabFurtherPlantDetails = (schema, allPlantInfo) => {
      ...
      const furtherDetails = furtherPlantDetails.reduce(
        (furtherDetails, topic) => {
          ...
          if (
            ['otherCommonNames', 'synonyms'].includes(makeCamelCase(topicTitle))
          ) {
    ...
    const grabPlantColours = (schema, allPlantInfo) => {
      ...
      colour.forEach(season => {
        ...
        colourBySeasonByType.forEach(li => {
          ...
          if (schema.colour[seasonNames[seasonTitle.innerText]][plantPartKey]) {
            schema.colour[seasonNames[seasonTitle.innerText]][
              plantPartKey
            ] = schema.colour[seasonNames[seasonTitle.innerText]][
              plantPartKey
    ...
    const grabPlantSunInfo = (schema, allPlantInfo) => {
      ...
      otherSunlightInfo.forEach(topic => {
        ...
        const topicDescriptionArray = topicDescription.join('').split(' or ');
    ...
    const grabPlantSize = (schema, allPlantInfo) => {
      ...
      sizeInfo.forEach(li => {
        ...
        schema.size[makeCamelCase(sizePropTitle)] = sizePropDescription.join(
          ''
        );
      });
      return schema;
...
// // * JSDOM setup for dom manipulation within NodeJS
  ...
  axios
    ...
    .then(res => {
      if (res.status === 200) {
        ...
        // ! very problematic issue with JSDOM... no support for innerText due to lack of contact with render engine
        ...
        console.log(plantLatinName.textContent.trim());
        plantSchema.plantLatinName = plantLatinName.innerText;
        plantSchema.plantCommonName = plantCommonName.innerText;
...
output('https://www.rhs.org.uk/Plants/18919/i-Viburnum-opulus-i/Details');
*/
...
page.open(
  ...
  status => {
    if (status !== 'success') {
      ...
      const allPlantInfo = page.evaluate(() => document.querySelector(`.ajaxSearchDetails`))
      console.log(foo);
    }
    phantom.exit();
  }
  ...
  const allPlantInfo = document.querySelector(`.ajaxSearchDetails`);
  console.log();
  console.log(allPlantInfo);
  let plantSchema = {};
  plantSchema.detailsPercentage = 0 / 9;
  const [plantLatinName, plantCommonName] = allPlantInfo.querySelector(
    '.ib'
  ).children;
  console.log(plantLatinName.innerText);
  plantSchema.plantLatinName = plantLatinName.innerText;
  plantSchema.plantCommonName = plantCommonName.innerText;
